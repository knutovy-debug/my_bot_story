...
ADMIN_ID = "8674877633"  # ТВОЙ ID (БЕЗ КАВЫЧЕК!)
...
def get_payment_keyboard():
    keyboard = [
        [InlineKeyboardButton("💳 Месяц (299 руб.)", callback_data="month_99")],
        [InlineKeyboardButton("💳 Год (1990 руб.)", callback_data="year_1990")],
    ]
...
async def payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = load_db()
    db["payment_data"][str(user_id)] = {"price": price, "period": period, "days": days}
    save_db(db)
...
# ======== ОБРАБОТКА "ОПЛАТИЛ" (УВЕДОМЛЕНИЕ ТЕБЕ) ========
async def payment_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "оплатил" in text.lower():
        db = load_db()
        payment_info = db["payment_data"].get(str(user_id), {"price": "Неизвестно", "period": "Неизвестно", "days": 30})
        ...
        await update.message.reply_text("✅ Ваш запрос отправлен администратору на подтверждение.")
    else:
        await update.message.reply_text("Сначала выберите тариф через кнопку «Оплатить доступ».")

# ======== ОБРАБОТКА ПОДТВЕРЖДЕНИЯ (ТЫ НАЖИМАЕШЬ КНОПКУ) ========
async def confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("confirm_"):
        db = load_db()
        payment_info = db["payment_data"].get(str(target_user), {"days": 30})
        ...
        await context.bot.send_message(chat_id=target_user, text=f"🎉 Ваш безлимитный доступ активирован на {days} дней!")
    elif data.startswith("reject_"):
        await query.edit_message_text(f"❌ Оплата от пользователя {target_user} отклонена.")
        await context.bot.send_message(chat_id=target_user, text="❌ Оплата отклонена. Попробуйте снова.")
...
async def handle_buttons(update, context):
    ...
    if text == "📖 Создать сказку":
        ...
        await update.message.reply_text("Напишите /start.", reply_markup=get_main_keyboard())
...
async def story_start(update, context):
    ...
    if not can_create_story(user_id):
        await update.message.reply_text(
            ...
            f"💳 Чтобы продолжить, нажмите «Оплатить доступ» в главном меню.\n"
            f"Карта: `{CARD_NUMBER}`",
            parse_mode="Markdown",
...
async def random_story(update, context):
    ...
    if not can_create_story(user_id):
        await update.message.reply_text(
            f"⚠️ Вы использовали все 3 бесплатные сказки.\n\n"
            f"💳 Нажмите «Оплатить доступ» в главном меню.",
            reply_markup=get_main_keyboard()
        )
        return -1
    context.user_data['conversation'] = True
