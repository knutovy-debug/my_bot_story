...
ADMIN_ID = 8434163956  # ТВОЙ TELEGRAM ID

openai.api_key = OPENAI_API_KEY

# ---- ССЫЛКА НА ОПЛАТУ (ЗАМЕНИ НА СВОЮ) ----
PAYMENT_LINK = "https://www.tinkoff.ru/..."  # ВСТАВЬ ССЫЛКУ НА ОПЛАТУ ИЛИ НОМЕР КАРТЫ

# ---- ОСТАЛЬНОЙ КОД (ГОЛОСА, ЯЗЫКИ, БД) ----
VOICES = {
...
async def help_command(update, context):
    await update.message.reply_text(
        "📖 Создать сказку – выбери имя, тему, длину, мораль, язык, характер, внешность и голос.\n"
        "📚 Мои сказки – список твоих сказок (можно удалить).\n"
        "💳 Оплатить доступ – снять лимит 3 сказки/день.\n"
        "❤️ Поддержать автора – реквизиты для доната.\n"
...
async def donate(update, context):
    await update.message.reply_text(
        ...
        f"🔗 Или по ссылке: {PAYMENT_LINK}\n\n"
        f"Ваша поддержка помогает создавать новые сказки!",
        parse_mode="Markdown",
        reply_markup=get_main_keyboard()
    )
...
async def payment(update, context):
    ...
    await update.message.reply_text(
        ...
        "После оплаты нажмите кнопку **«Я оплатил(a)»**.\n"
...
async def story_start(update, context):
    ...
    if not can_create_story(user_id):
        await update.message.reply_text(
            ...
            f"💳 Нажмите «Оплатить доступ», чтобы снять лимит.\n"
...
async def story_voice(update, context):
    ...
    prompt = {
        ...
        'de': f"Schreibe ein {length} Märchen für ein 5-7 jähriges Kind auf Deutsch. Held – {name}, {trait}, {appearance}. Thema: {topic}. Moral: {moral}.",
        'fr': f"Écris un conte {length} pour un enfant de 5-7 ans en français. Héros – {name}, {trait}, {appearance}. Thème: {topic}. Moral: {moral}.",
        'it': f"Scrivi una fiaba {length} per un bambino di 5-7 anni in italiano. Eroe – {name}, {trait}, {appearance}. Tema: {topic}. Morale: {moral}.",
        'zh': f"为5-7岁儿童写一个{length}的童话故事，中文。主角–{name}，{trait}，{appearance}。主题：{topic}。寓意：{moral}。",
    }.get(language, "ru")
...
# ---- ОБРАБОТЧИК ИНЛАЙН-КНОПОК (включая подтверждение) ----
async def handle_inline(update, context):
    ...
    if data == "new_story":
        ...
        await update.effective_user.send_message(
            f"💳 Пользователь @{update.effective_user.username} (ID: {user_id}) оплатил доступ.\n"
            f"Подтвердить?",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        ...
        try:
            await update.effective_bot.send_message(
                user_id,
                "🎉 Ваш безлимитный доступ активирован!\n"
                "Теперь вы можете создавать сколько угодно сказок!"
            )
        except:
            pass
        ...
        try:
            await update.effective_bot.send_message(
                user_id,
                "❌ Ваш запрос на оплату был отклонён администратором.\n"
